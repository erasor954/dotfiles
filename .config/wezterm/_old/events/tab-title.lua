...
--local GLYPH_SCIRCLE_RIGHT = nf.ple_upper_left_triangle --[[  ]]
...
local function get_process_icon(proc)
	return process_icons[proc] or process_icons["default"]
end

local ICON_DEFAULT_PROCESS = nf.md_application -- Fallback icon
...
local RENDER_VARIANTS = {
	...
	{ "scircle_left", "admin", "index", "title", "padding", "scircle_right" },
	{ "scircle_left", "admin", "index", "title", "unseen_output", "padding", "scircle_right" },
	{ "scircle_left", "wsl", "index", "title", "padding", "scircle_right" },
	{ "scircle_left", "wsl", "index", "title", "unseen_output", "padding", "scircle_right" },
}
...
function Tab:set_info(event_opts, tab, max_width)
	self.index = tab.tab_index + 1

	-- 1. Get the clean process name (e.g., "nvim")
	...
	-- Pass the new process_name (with icon) to create_title
...
function Tab:update_cells(event_opts, is_active, hover)
	...
	local index_icon_set = GLYPH_INDEX_OUTLINE

	if is_active then
		tab_state = "active"
		index_icon_set = GLYPH_INDEX_FILLED
	elseif hover then
		tab_state = "hover"
	end
	...
	self.cells:update_segment_text("index", " " .. index_icon_set[idx])
	...
	self.cells
		...
		:update_segment_colors("index", colors["text_" .. tab_state])
